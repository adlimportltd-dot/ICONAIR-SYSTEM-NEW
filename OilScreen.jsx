...
import {
  listOilEntries, createOilEntry, listDeviceOptions, getOilByScent,
} from '../lib/queries';
...
function NewOilEntryModal({ open, deviceOptions, devices, onClose, onCreated }) {
  ...
  return (
    <Modal
      open={open}
      title="רישום מילוי שמן"
      subtitle="הרישום מעדכן אוטומטית את מפלס השמן של המכשיר"
      onClose={onClose}
    >
      <form onSubmit={submit} className="flex flex-col gap-3.5">
        ...
        <div className="grid grid-cols-1 gap-3.5 xs:grid-cols-2">
          ...
          <Field label="ניחוח" hint="השאר ריק כדי לא לשנות את הניחוח הנוכחי במכשיר">
            <TextInput value={form.scent_name} onChange={set('scent_name')} placeholder="ללא שינוי" />
          </Field>
        ...
        <div className="mt-1 flex gap-2.5">
          <PrimaryButton type="submit" loading={busy}>שמור רישום</PrimaryButton>
